...
module.exports = function(oAccessory, oService, oCharacteristic, ouuid) {
    if (oAccessory) {
        ...
        communityTypes = require('../lib/communityTypes').CommunityTypes(Characteristic);
        ...
        if (device && device.capabilities) {
            ...
            if (isSpeaker === true) {
                ...
                if (that.device.attributes.status !== undefined) {
                    thisCharacteristic = that.getaddService(communityServices).getCharacteristic(communityTypes.PlaybackState);
                    thisCharacteristic.on('get', function(callback) {
